...
| Duplicate dispatch/retry arrives | `claim_next_job` admits only active runs, increments a persisted fencing token, and `execute_claimed_job` locks and rechecks before provider work. The old-worker fencing test proves a reclaimed lease cannot write. The preclaimed terminal-run race preserves the original terminal lifecycle. PASS. |
...
| Admin and MCP inspect one record | Admin recovery summary and `execution.get_run_status` consume one safe queue, lease/fencing, recovery, workspace/retention, and evidence projection; MCP test coverage is green. PASS. |
...
| Check | Result |
| --- | --- |
| Focused lifecycle suite | `58 passed` |
| Repository test suite: `pytest` | PASS â€” `197 passed in 10.56s` |
| `ruff check .` | PASS |
