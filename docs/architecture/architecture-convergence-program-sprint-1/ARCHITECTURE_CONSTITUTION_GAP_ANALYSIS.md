...
| Engine / State Machine Constitutions | Strong bounded-context and lifecycle-separation basis. | No Capability Provider contract; no explicit rule that execution state lives outside an Engine. | Módosítani | Add capability declaration/resolution and operational-state prohibition. |
...
Each arrow must carry correlation, scope, policy and evidence references. A
Capability Provider may be an Engine, Tool, Provider or Agent; it is not the
authority that owns Mission or operational Execution state.
...
target Execution aggregate. Those choices require ADRs before implementation.
