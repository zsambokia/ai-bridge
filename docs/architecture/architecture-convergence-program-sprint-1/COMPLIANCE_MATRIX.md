...
| Recovery remains within the bound Provider | Existing recovery behavior is not yet constrained by a Provider Binding and Kernel Profile contract. | Gap | Módosítani / ADR szükséges | ADR-029 and Phase 3; provider outage must be explicit, never an automatic cross-provider failover. |
