...
| `docs/architecture/ARCHITECTURE_CONSTITUTION.md`, `ARCHITECTURE_MAP.md` | Core technical law and topology. | Módosítani | Preserve laws; add intake, capability, execution, context, scope and locale chapters/links. |
...
| `docs/architecture/ADRs/*` | Current accepted decisions. | Megtartani / Új elem | Preserve accepted ADRs; add ADR-020–028 only through later approval. |
| `projects/models.py` Project/Repository-related models | Project-centric identity. | Új elem / ADR szükséges | Add scoped Organization/logical Workspace/Repository ownership only after hierarchy decision; avoid collision with `ExecutionWorkspace`. |
| `projects/models.py` `ExecutionRun`, jobs, recovery, delivery, progress events | Reusable operational mechanics. | Migrálni | Bind to target Execution; retain attempt/recovery semantics. |
...
| `projects/execution.py`, provider gateway/provider models | Execution/provider transport mechanisms. | Megtartani / Migrálni | Reuse in Operational Foundation boundary; target contract must own policy/context binding. |
