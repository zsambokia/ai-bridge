...
class CanonicalFactoryAcceptanceSuiteTests(TestCase):
    ...
    def _real_engineering_operation(self, repository: Path, *, repair: bool) -> dict[str, object]:
        app = repository / "app.py"
        app.write_text("def result():\n    return 'ok'\n" if repair else "def result(:\n", encoding="utf-8")
        build = subprocess.run(
            [sys.executable, "-m", "py_compile", str(app)], cwd=repository,
        ...
        diff = subprocess.run(
            ["git", "diff", "--", "app.py"], cwd=repository,
            capture_output=True, text=True, check=False,
        )
        return {
            ...
            "verification": {
                ...
                "checks": {"app.py changed": bool(diff.stdout), "build passes": True, "regression passes": True},
            },
    ...
    def test_level_2_engineering_mission_repairs_retries_builds_and_retests(self) -> None:
        execution = self._approved_execution()
        with TemporaryDirectory() as directory:
            repository = Path(directory)
            subprocess.run(["git", "init"], cwd=repository, capture_output=True, check=True)
            (repository / "app.py").write_text("def result():\n    return 'broken'\n", encoding="utf-8")
            subprocess.run(["git", "add", "app.py"], cwd=repository, capture_output=True, check=True)
            subprocess.run(
                ["git", "-c", "user.email=acceptance@example.invalid", "-c", "user.name=Acceptance", "commit", "-m", "baseline"],
                cwd=repository, capture_output=True, check=True,
            )
            execute_shadow_operation(str(execution.token), actor=self.actor,
                                     operation=lambda: self._real_engineering_operation(repository, repair=False))
            ...
            execute_shadow_operation(str(execution.token), actor=self.actor,
                                     operation=lambda: self._real_engineering_operation(repository, repair=True))

        execution.refresh_from_db()
        self.assertEqual(execution.state, OrkiExecution.State.COMPLETED)
        self.assertEqual(execution.plan.goal.status, "ACHIEVED")
        self.assertTrue(execution.events.filter(event_type="EXECUTION_ATTEMPT_FAILED").exists())
        self.assertTrue(execution.events.filter(event_type="verification.completed", payload__passed=True).exists())

    def test_level_3_factory_goal_persists_plan_graph_evidence_and_reflection_before_knowledge(self) -> None:
        execution = self._approved_execution()
        self.assertEqual(len(execution.plan.strategy_references["mission_graph"]), len(self.checks))
        with TemporaryDirectory() as directory:
            repository = Path(directory)
            subprocess.run(["git", "init"], cwd=repository, capture_output=True, check=True)
            (repository / "app.py").write_text("def result():\n    return 'broken'\n", encoding="utf-8")
            subprocess.run(["git", "add", "app.py"], cwd=repository, capture_output=True, check=True)
            subprocess.run(
                ["git", "-c", "user.email=factory@example.invalid", "-c", "user.name=Factory", "commit", "-m", "baseline"],
                cwd=repository, capture_output=True, check=True,
            )
            execute_shadow_operation(str(execution.token), actor=self.actor,
                                     operation=lambda: self._real_engineering_operation(repository, repair=True))
        ...
        execution.refresh_from_db()
        entry = KnowledgeEntry.objects.get(entry_key="factory-acceptance-repair-pattern")
        event_types = list(execution.events.order_by("sequence").values_list("event_type", flat=True))
        self.assertEqual(execution.state, OrkiExecution.State.COMPLETED)
        ...
        self.assertLess(event_types.index("reflection.completed"), event_types.index("knowledge.candidate.created"))
        self.assertNotIn("embedding.generated", event_types)
        self.assertTrue(all(event.evidence_references for event in execution.events.all()))
